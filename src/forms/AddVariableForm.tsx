...
export const AddVariableForm = ({
  ...
  const mutation = useMutation({
    mutationFn: (data: VariableFormInput) => {
      ...
      return TemplateAPI.configTemplate(projectId, templateId, {
        variables: updatedVariables,
      });
    },
    onSuccess: (res) => {
      toast.success(res.data?.message ?? "Variable added successfully", {
        position: "top-right",
      });
    ...
    onError: (error) => {
      const msg = getErrorMessage(error) || "Failed to add variable.";
      toast.error(msg, { position: "top-right" });
    },
  });

  const onSubmit = (data: VariableFormInput) => {
    // console.log("Submitting variable:", data);
